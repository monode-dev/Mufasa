...
export type ReadonlyProp<T> = { readonly value: T }
type IsFunction<T> = T extends (...args: any[]) => any ? true : false
export function prop<
  T,
  Setter extends T extends (...args: any[]) => infer R ? (newValue: R) => void : never = never,
>(
  initValue: T,
  setter?: Setter,
): IsFunction<T> extends true
  ? IsFunction<Setter> extends true
    ? Prop<ReturnType<T extends (...args: any[]) => any ? T : never>>
    : ReadonlyProp<ReturnType<T extends (...args: any[]) => any ? T : never>>
  : Prop<T> {
...
export function propFromFuncs<T, Set extends ((newValue: T) => void) | undefined = undefined>(
  get: () => T,
  set?: Set,
...
export function propOrFallback<T extends Prop<any> | ReadonlyProp<any>>(
  getPreferredProp: () => T | null | undefined,
  fallback: T,
): T {
  return prop<() => T[`value`], (newValue: T[`value`]) => void>(
    () => (getPreferredProp() ?? fallback).value,
    newValue => (((getPreferredProp() ?? fallback) as any).value = newValue),
  ) as any
}

// TODO: Maybe name Formula
// export function prop<T>(getter: () => T): ReadonlyProp<T> {
...
//   }
// }
export function watchOnly(deps: ReadonlyProp<any>[], callback: () => void) {
  return createEffect(
    on(

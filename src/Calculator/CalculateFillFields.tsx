...
export function CalculateFillFields(props: { subDelivery?: SubDelivery }) {
  ...
  const stickedInchesBeforeFilling = doNow(() => {
    const _localStickedInches = useProp<number | null | undefined>(undefined);
    return useFormula(
      () =>
        exists(selectedSubDelivery.value)
          ? selectedSubDelivery.value.stickedInchesBeforeFilling
          : _localStickedInches.value,
      (val) =>
        exists(selectedSubDelivery.value)
          ? (selectedSubDelivery.value.stickedInchesBeforeFilling = val ?? null)
          : (_localStickedInches.value = val),
    );
  });
  const stickedInchesHasFocus = useProp(false);
  const focusOnStickedInchesIfEmpty = () => {
    if (!exists(stickedInchesBeforeFilling.value)) {
  ...
  function fillOutline(val: number | undefined) {
    if (!exists(val) || isNaN(val))
      if ((stickedInchesBeforeFilling.value ?? 0) > 0)
        return $theme.colors.error;
      ...
      >
        <NumField
          value={useFormula(
            () => stickedInchesBeforeFilling.value,
            (v) => {
              stickedInchesBeforeFilling.value = v;
            },
          )}
